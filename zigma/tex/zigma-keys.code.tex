...
\cs_new_protected:Npn \zigma_apply_setup:
  {
    ...
    % Step 4: Build canonical license text (used by all renderers)
    \zigma_meta_license_render_build:

    % Step 5: Display configuration state (debug mode only)
    \bool_if:NT \g_zigma_debug_bool { \zigma_log_state_soft: }
    
    % Step 6: Display registered authors (debug mode only)

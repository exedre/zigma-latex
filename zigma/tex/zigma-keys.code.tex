...
\cs_new_protected:Npn \zigma_apply_setup:
  {
    ...
    % Step 5: Apply dropcap policy (neutralize \lettrine when disabled)
    \zigma_apply_dropcap_policy:

    % Step 6: Build canonical license text (used by all renderers)
    \zigma_meta_license_render_build:

    % Step 7: Build canonical publication string
    \zigma_meta_published_render_build:

    % Step 8: Display configuration state (debug mode only)
    \bool_if:NT \g_zigma_debug_bool { \zigma_log_state_soft: }
    
    % Step 9: Display registered authors (debug mode only)

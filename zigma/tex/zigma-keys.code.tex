...
\cs_new_protected:Npn \zigma_apply_setup:
  {
    ...
    % Step 5: Build canonical license text (used by all renderers)
    \zigma_meta_license_render_build:

    % Step 6: Build canonical publication string
    \zigma_meta_published_render_build:

    % Step 7: Display configuration state (debug mode only)
    \bool_if:NT \g_zigma_debug_bool { \zigma_log_state_soft: }
    
    % Step 8: Display registered authors (debug mode only)
